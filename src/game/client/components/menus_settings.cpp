...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	else if(g_Config.m_UiSettingsPage == SETTINGS_XECLIENT)
	{
		m_pBackground->ChangePosition(CMenuBackground::POS_SETTINGS_XECLIENT);
		RenderSettingsXEClient(MainView);
	}
	else if(g_Config.m_UiSettingsPage == SETTINGS_XEBOT)
	{
		m_pBackground->ChangePosition(CMenuBackground::POS_SETTINGS_XEBOT);
		RenderSettingsXEBot(MainView);
	}
	else if(g_Config.m_UiSettingsPage == SETTINGS_XEPROFILES)
	{
		m_pBackground->ChangePosition(CMenuBackground::POS_SETTINGS_XEPROFILES);
		RenderSettingsXEProfiles(MainView);
	}
...
void CMenus::RenderSettingsXEClient(CUIRect MainView)
{
	CUIRect Button, Left, Right, Label;
}

void CMenus::RenderSettingsXEBot(CUIRect MainView)
{
	CUIRect Button, Label, Left, Right, Box, 自动瞄准;

	MainView.HSplitTop(200, &自动瞄准, &MainView);

	自动瞄准.HSplitTop(50.0f, &Label, &自动瞄准);
	UI()->DoLabel(&Label, Localize("自动瞄准"), 20.0f, TEXTALIGN_ML);
	自动瞄准.HSplitTop(50.0f, &Button, &自动瞄准);
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_XEAimBot, Localize("自瞄开关"), &g_Config.m_XEAimBot, &Button, 20.0f);
	自动瞄准.HSplitTop(50.0f, &Box, &自动瞄准);
	Box.VSplitLeft(自动瞄准.w - 5.0f, &Box, &Right);
	Box.Draw(ColorRGBA(1, 1, 1, 0.5), IGraphics::CORNER_ALL, 3.0f);
	自动瞄准.HSplitTop(50.0f, &Box, &自动瞄准);
	Box.VSplitLeft(自动瞄准.w - 5.0f, &Box, &Right);
	Box.Draw(ColorRGBA(1, 1, 1, 0.5), IGraphics::CORNER_ALL, 3.0f);
}

void CMenus::RenderSettingsXEProfiles(CUIRect MainView)
{
}
